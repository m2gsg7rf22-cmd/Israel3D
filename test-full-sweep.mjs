...
await page.evaluate(() => document.getElementById('settings-weapons').click());
const weaponRows = await page.evaluate(() => document.querySelectorAll('#weapon-list .dealer-item').length);
if (weaponRows < 1) throw new Error('expected weapon shop rows, got ' + weaponRows);
await page.evaluate(() => document.querySelector('#panel-weapons .panel-close').click());
await page.evaluate(() => document.getElementById('settings-radio').click());
await page.evaluate(() => document.getElementById('settings-radio').click());
...
log('settings buttons (mute/skip-time/weapon-shop/radio/touch-size/difficulty)');
